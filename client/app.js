...
async function login() {
    ...
    try {
        if (!socket || socket.disconnected) {
            // УНИВЕРСАЛЬНОЕ ПОДКЛЮЧЕНИЕ - работает везде
...
async function register() {
    ...
    try {
        if (!socket || socket.disconnected) {
            // ТАКОЕ ЖЕ ПОДКЛЮЧЕНИЕ КАК В login()
            const socketHost = window.location.origin.replace(/:\d+$/, '') + ':3000';
...
function addMessageToChat(message) {
    ...
    if (isFileMessage && message.fileUrl) {
        ...
        const isAudio = message.isAudio || message.fileType?.startsWith('audio/'); // <-- ИСПРАВЛЕНО: объявляем переменную
        ...
        if (isImage) {
            ...
            `;
        } else if (isAudio) { // <-- Теперь переменная определена
...
function formatMessageText(text) {
    ...
    Object.keys(smileyMap).forEach(smiley => {
        formattedText = formattedText.replace(new RegExp(smiley.replace(/[.*+?^${}()|[\]\\]/g, '\\$&'), 'g'), smileyMap[smiley]);
    });
    
    formattedText = formattedText.replace(/@(\w+)/g, '<span class="mention">@$1</span>');
    formattedText = formattedText.replace(/(https?:\/\/[^\s]+)/g, '<a href="$1" target="_blank" class="message-link">$1</a>');
...
function adminEditProfile(username) {
    alert('Редактирование профиля пользователя будет реализовано в следующем обновлении');
}
            ...
            <div class="modal-body">
                <div class="settings-section">
                    ...
                    <div class="form-group">
                        ...
                        <select id="setting-font-size" class="select-custom">
                ...
                <div class="settings-section">
                    ...
                    <div class="form-group">
                        <label class="checkbox">
                            <input type="checkbox" id="setting-message-preview">
                            <span>Показывать превью сообщений</span>
                        </label>
                    </div>
                </div>
                
                <div class="settings-section">
                    <h4><i class="fas fa-shield-alt"></i> Конфиденциальность</h4>
                    <div class="form-group">
                        <label>Кто может писать вам:</label>
                        <select id="setting-privacy" class="select-custom">
                    ...
                    <div class="form-group">
                        <label class="checkbox">
                            ...
                            <span>Отправлять подтверждения прочтения</span>
                        </label>
                    </div>
                </div>
                
                <div class="settings-section">
                    <h4><i class="fas fa-language"></i> Язык и регион</h4>
                    <div class="form-group">
                        <label>Язык интерфейса</label>
                        <select id="setting-language" class="select-custom">
                    ...
                    <div class="form-group">
                        ...
                        <select id="setting-time-format" class="select-custom">
                ...
                <div class="settings-section">
                    <h4><i class="fas fa-download"></i> Медиафайлы</h4>
                    <div class="form-group">
                        <label class="checkbox">
                            <input type="checkbox" id="setting-auto-download">
                            <span>Автоматически загружать медиа</span>
                        </label>
                    </div>
                    <div class="form-group">
                        <label>Качество изображений:</label>
                        <select id="setting-image-quality" class="select-custom">
                    ...
                    <div class="form-group">
                        ...
                        <select id="setting-video-quality" class="select-custom">
                            <option value="high">Высокое</option>
                            <option value="medium" selected>Среднее</option>
                            <option value="low">Низкое</option>
                        </select>
                ...
                <div class="settings-actions">
                    ...
                    <button id="reset-settings-btn" class="secondary-btn">
...
function saveSettings() {
    ...
    const settings = {
        ...
        autoDownload: document.getElementById('setting-auto-download')?.checked || false,
        messagePreview: document.getElementById('setting-message-preview')?.checked || false,
        privacy: document.getElementById('setting-privacy')?.value || 'everyone',
        onlineStatus: document.getElementById('setting-online-status')?.checked || true,
        readReceipts: document.getElementById('setting-read-receipts')?.checked || false,
        language: document.getElementById('setting-language')?.value || 'ru',
        timeFormat: document.getElementById('setting-time-format')?.value || '24',
        imageQuality: document.getElementById('setting-image-quality')?.value || 'medium',
        videoQuality: document.getElementById('setting-video-quality')?.value || 'medium',
...
function initAudioUpload() {
    ...
    // Добавляем кнопку в input-actions
...
async function processAudioUploadQueue() {
    ...
    console.log('🎵 Начало загрузки аудио:', file.name);
    ...
    try {
        ...
        if (response.ok) {
            const result = await response.json();
            console.log('✅ Аудио загружено на сервер:', result);
            ...
            console.log('🔗 Полный URL:', fullAudioUrl);
            
            const audioData = {
                url: fullAudioUrl, // Используем полный URL
            ...
            console.log('📦 Данные для отправки:', audioData);
            
            // Пробуем воспроизвести для теста
            testAudioPlayback(fullAudioUrl);
...
// Функция для тестирования воспроизведения
function testAudioPlayback(audioUrl) {
    console.log('🔊 Тест воспроизведения:', audioUrl);
    ...
    testAudio.onloadedmetadata = () => {
        ...
        console.log('⏱️ Длительность:', testAudio.duration);
    };
    
    testAudio.onerror = (e) => {
        console.error('❌ Ошибка загрузки аудио:', e);
        console.error('Аудио URL:', audioUrl);
    };
